...
class DivisionBenchmark {
    ...
    fun benchmarkSmallOperands(seed: Int) = runBlockingTest {
        ...
        for (i in 0 until numberOfSamples) {
            jobList += GlobalScope.launch {
                val a = ULongArray(2) { random.nextULong() shr 1 }.toProperType()
    ...
    fun benchmarkRangeOfOperands(seed: Int) = runBlockingTest {
        ...
        for (i in 0 until numberOfSamples) {
            jobList += GlobalScope.launch {
                val a = ULongArray(i * 2) { random.nextULong() shr 1 }.toProperType()
    ...
    fun benchmarkLargeOperands(seed: Int) = runBlockingTest {
        ...
        val operandSize = 8_000
        val jobList = mutableListOf<Job>()
        for (i in 0 until numberOfSamples) {
            jobList += GlobalScope.launch {
                val a = ULongArray(operandSize * 2) { random.nextULong() shr 1 }.toProperType()
    ...
    fun runDivision(first: WordArray, second: WordArray): Long {
        return measureTime {
            chosenArithmetic.divide(first, second)
