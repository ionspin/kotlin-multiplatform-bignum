...
class BigInteger63LinkedListJavaMultiplyTest {
    ...
    @Test
    fun multiplyTwoLargeWordsTests() {
        val seed = 1
        val random = Random(seed)
    ...
    fun `Test multiplying a lot of words`() {
        ...
        multiplySingleTest(*first.toULongArray())
    }

    @Test
    fun preciseMultiplyTest() {
        multiplySingleTest(
            listOf(3751237528UL, 9223372035661198284UL, 7440555637UL, 0UL, 2UL, 0UL, 2UL),
            listOf(1UL, 1UL)
        )
//        multiplySingleTest(1193170172382743678UL, 17005332033106823254UL, 15532449225048523230UL) Invalid sample, 64 bit number!
    ...
    fun multiplySingleTest(first: List<ULong>, second: List<ULong>) {
        assertTrue(
            "Failed on listOf(${first.joinToString(separator = ", ") { it.toString() + "UL" }})," +
    ...
    fun twoWordsSingleMultiplication(first: ULong, second: ULong) {
        val bigIntResult = BigInteger63LinkedListArithmetic.multiply(first, second)
    ...
    fun karatsubaSingleTest(first: List<ULong>, second: List<ULong>) {
        val bigIntResult =
            BigInteger63LinkedListArithmetic.karatsubaMultiply(first, second)
        val javaBigIntResult = first.toJavaBigInteger() * second.toJavaBigInteger()
